...
public class SlideBackLayout extends FrameLayout {
  ...
  public SlideBackLayout(@NonNull Context context, @Nullable AttributeSet attrs, int
      defStyleAttr) {
    super(context, attrs, defStyleAttr);
    TypedArray array = context.obtainStyledAttributes(attrs, R.styleable.SlideBackLayout);
    boolean leftEnable = array.getBoolean(R.styleable.SlideBackLayout_left_swipe, true);
    boolean rightEnable = array.getBoolean(R.styleable.SlideBackLayout_right_swipe, true);
    array.recycle();
  ...
  public boolean onInterceptTouchEvent(MotionEvent ev) {
    ...
    return (mIsLeftStart && mIsLeftSwipeEnable) || (mIsRightStart && mIsRightSwipeEnable) ||
  ...
  private void drawSinLine(Canvas canvas) {
    ...
    float amplitude = initAmplitude * progress * 1.5f;
    float height = initAmplitude * 2 * progress * 1.5f;
    float sineWidth = dip2px(135);
    float sineIndex = 0;
    float sineTheta = 30;
    float valueSineStart = 0;
    float sineLineStartY = mStartY - 1.9f / 3f * sineWidth;
    if (mIsRightStart) {
    ...
    while (sineIndex <= sineWidth * 4 / 3) {
      valueSine = (float) (Math.sin(sineIndex / sineWidth * 1.5f * Math.PI + sineTheta)
    ...
    if (mIsRightStart) {
      ...
      canvas.drawLine(midBackX - lineLength, midBackY, midBackX, midBackY - lineLength,
          mPaintWhite);
      canvas.drawLine(midBackX - lineLength, midBackY, midBackX, midBackY + lineLength,
          mPaintWhite);
    } else {
