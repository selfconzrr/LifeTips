...
public class Draw_Line extends DrawUtil {

  private Point cenPoint; // 声明线段的中间点，拖动直线需要用到该点
    ...
    switch (event.getAction()) {
      ...
      case MotionEvent.ACTION_UP:
        ...
        lPoint2Rect.set(lPoint2.x - 25, lPoint2.y - 25, lPoint2.x + 25,
            lPoint2.y + 25);
        Log.e("zhangrr", "onTouchEvent() called with: event = ACTION_UP``````````````break");
        break;
      default:
        break;
    }
   ...
   * 与线段的距离进行比较 来判断
   */
  public boolean isPointInLine() {

    double lDis = Math.sqrt((lPoint1.x - lPoint2.x) * (lPoint1.x - lPoint2.x)
        + (lPoint1.y - lPoint2.y) * (lPoint1.y - lPoint2.y)); // 线段的长度
